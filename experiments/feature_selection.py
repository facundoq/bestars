...
class BinaryFeatureSelection(StarExperiment):
    ...
    def eval_combinations(self, x, y, column_names, dataset_name, feature):
        ...
        if self.verbose:
            print(
                f"Evaluating {len(paired_indices)} binary features combinations:")
        ...
        for pair in paired_indices:
            ...
            score = self.evaluate_model(
                x_train_pair, x_test_pair, y_train, y_test)

            kbest.update(score, pair)
            qa_label, qb_label = [column_names[c] for c in pair]
            qdf = pd.DataFrame([[qa_label, qb_label, score]],
    ...
    def run(self):
        ...
        for combination_size in [3, 4]:
            ...
            q = qfeatures.calculate(
                x_np, coefficients_np, x.columns, systems, combination_size=combination_size)
            self.eval_combinations(
                q.magnitudes, y_np, q.column_names, dataset_name, feature)
    ...
    def evaluate_model(self, x_train, x_test, y_train, y_test):
        ...
        clf = MLPClassifier(solver='adam', alpha=1e-3,
                            hidden_layer_sizes=(32, 16, 8), random_state=1, max_iter=1000)
